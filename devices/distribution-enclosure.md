...
> **Interim placement.** The *Proxied PV Child* and *Proxied EVSE Child* subsections below are partial PV / EVSE data models, framed as "what the enclosure publishes when proxying." Rules they state are properly per-device-data-model statements, not enclosure-spec statements, and will move to `devices/pv.md` and `devices/evse.md` when those per-device data models land. (The BESS equivalent has already moved to [`devices/bess.md`](bess.md); the general proxy-publication conventions live in [`devices/proxy.md`](proxy.md).)
