...
The document headers are the single source of truth for artifact versions. This
script aggregates them into (1) a machine-readable manifest that downstream drift
checks consume, and (2) the human-readable README status table, so the two can
never drift from the headers or from each other. Run it after any version bump;
`--check` fails if either output is stale (suitable for CI or a pre-push check).
...
# Areas scanned (label for the README table, subdirectory). Index README.md files
# are navigation, not versioned artifacts, and are excluded from both outputs.
...
# Non-versioned placeholder rows (planned docs with no file yet).
PLACEHOLDERS = {
...
def manifest_json(artifacts):
    return json.dumps({"spec_repo": SPEC_REPO,
        "note": "Generated from document Version:/Date:/Status: headers by tools/gen-spec-manifest.py. Do not edit by hand.",
...
def main():
    ...
    new_manifest = manifest_json(artifacts)
