...
A grid event is a *signal*: it says what the grid is asking, not what any one device must physically do. How the site responds is a separate concern: an EMS decomposes the event and drives its flexible loads through their own device-level [`dr`](../data-models/water-heater.md) control surface, and compliance is reported through `settlement-proof`. See [§Relationship to other signals](#relationship-to-other-signals).
...
- `mode`: the direction of the ask, reused from the device-level [`dr`](../data-models/water-heater.md) vocabulary. `SHED` asks the site to reduce net consumption (curtail load or discharge storage); `LOAD_UP` asks it to increase net consumption (absorb surplus / cheap energy); `NORMAL` is an all-clear that ends an event. Required.
...
- `level`: the magnitude of the ask as a percentage (for events that quantify one), reused from `dr`. Absent for an unquantified alert (a conservation call carries no percentage).
...
The response side lives elsewhere: an EMS decomposes a `grid-event` and drives each flexible load through its device-level [`dr`](../data-models/water-heater.md) control surface (settable `dr/event`, with the load reporting `dr/dr-response` and `dr/opted-out`), and site-aggregate compliance is emitted as `settlement-proof`. `grid-event` (the site-wide ask) and `dr` (the per-load control + feedback) are the two ends of demand response; this capability is the ask, not the doing.
...
- [water-heater](../data-models/water-heater.md) data model — defines the device-level `dr` control + feedback surface that acts on these events.
