...
- **convention/property-json** (0.1 -> 0.2): documented **abstract unit tokens**. Four properties carry `"unit": "energy"` rather than a concrete unit (`soc`'s `soe`, `total-energy-storage` and `loadup-headroom`, and `info`'s `nameplate-capacity`), because a reservoir's stored-energy magnitudes are reported in the device's native energy unit: electrical `kWh` for a BESS, thermal `Wh` for a water heater, neither the same unit nor summable. That was stated only in `soc.md`'s prose, so nothing told a reader of the machine-readable catalogs that `"unit": "energy"` is a dimension rather than a unit, and `"unit": "energy"` and `"unit": "kWh"` were indistinguishable in shape. The convention now carries a token table and two rules that follow from it: a publisher MUST substitute a concrete unit in its runtime `$description` (the token is never published on the wire), and a consumer MUST read the unit from the runtime `$description` rather than the catalog for any property carrying a token, or it renders the literal string `energy`. `tools/check-property-catalogs.py` gains `ABSTRACT_UNITS` and `CONCRETE_UNITS` and now emits an advisory note for any unit that is neither, so a typo such as `enrgy` surfaces instead of passing silently as an opaque string; the current corpus produces zero notes. Advisory, never fatal, consistent with the descriptive-not-prescriptive stance: the note nudges, it does not constrain what a conformant publisher may do. Found while auditing a downstream implementation against the catalogs, via [python-sdk#27](https://github.com/electrification-bus/python-sdk/issues/27). ([#7](https://github.com/electrification-bus/specification/issues/7))
...
- **convention/property-json**: device profiles now cover every device model. Added `outlet`, `pdu`, `utility-meter`, and `water-heater` (`devices/*.json`) alongside `bess`, `circuit`, and `distribution-enclosure`; `proxy.md` has no profile (it is the proxy-publication convention and declares no device type). No version change: the profiles are generated siblings, co-versioned with their prose.
...
- **capability/load-shed** 0.2 → 0.3: `load-shed/priority` is now **settable**. The catalog previously carried no settable attribute (read-only by default), but the property is intended to be writable so a consumer or host controller can set a circuit's shed class over the bus (for example, assigning which circuits shed first, or marking a circuit `NEVER`). Added the Settable column (`priority` = yes) and a note that a publisher unable to accept writes for internal reasons advertises it read-only (`$settable: false`) in its `$description`, a permitted deviation under the framework's conformance latitude. Additive for consumers (those that treated `priority` as read-only are unaffected); implementers of `load-shed` re-pin to 0.3, and the generated `circuit.json` load-shed `catalog_version` follows to 0.3.
- **capability/info** 0.1 → 0.2: added two optional identity / rating properties, and a Unit column to the identity table. `part-number` (string, MAY): a manufacturer part or SKU number that MAY differ from `model`, since distinct part numbers can designate the same model. `nameplate-capacity` (float, MAY, device-native energy unit): the rated nameplate energy capacity, a term of art for energy-storage devices (a BESS in kWh electrical, a storage water heater in Wh thermal); energy-storage device types SHOULD publish it, while power-rated devices (PV, inverter, EVSE) publish rated power via `nominal-power` instead. `nameplate-capacity` is the static rated reference for state of charge (`soc` is roughly `soe` / `nameplate-capacity`), distinct from the dynamic `total-energy-storage` in `soc` that the two diverge from as the reservoir degrades. It is promoted from a device-specific `info` property in `bess.md` to the shared core (BESS's `nominal-power` stays BESS-specific). Also tightened the `model` vs `part-number` distinction: `model` is the human-facing model or product designation (e.g. "Powerwall 2 AC", or a configuration code like "MAIN_32"), while `part-number` is the orderable part / SKU code (e.g. "1232100-00-E"); the device-specific `product-name` in `bess.md` is retired, since a product designation belongs in `model` and a metering-point role label in the device's Homie `$name`. Additive; implementers of `info` re-pin to 0.2, and every device profile's `info` `catalog_version` follows automatically.
...
- **convention/property-json** 0.1: a new convention defining machine-readable property definitions. Each versioned capability catalog and data-model document gains a co-located JSON sibling (`capabilities/<name>.json`, `devices/<name>.json`) generated from and verified against the prose. The JSON is descriptive, not prescriptive (per the framework Design Principles): a `capability-catalog` is the recommended, extensible property vocabulary for a capability (how to publish each property if you do; MAY by default, with datatypes and enum value sets a publisher may widen or redefine and advertise in `$format`), and a `device-profile` is a light, advisory composition of which capabilities each device type publishes plus the spec's capability-level Req guidance where stated. A device's runtime Homie `$description` remains authoritative for what it actually publishes. The JSON inherits its prose file's `Version:` (so an existing artifact pin covers both files and no new pin key is introduced), and stamps a `schema_version` token whose contract is this convention. See [`conventions/property-json.md`](conventions/property-json.md).
...
- **capability/meter** — corrected the reactive-power and reactive-energy unit strings from `VAR` / `VARh` to the lowercase `var` / `varh`: the IEC 80000-6 standardized symbol, and the casing the [Homie convention](https://github.com/electrification-bus/convention)'s recommended-unit list uses for SI symbols (`W`, `Hz`, `Pa`, ...). This also removes an internal inconsistency: `devices/bess.md` already used the lowercase `var` (its Enphase mapping notes the source is "already var"). Apparent power/energy (`VA` / `VAh`) were already correct and are unchanged. In-place DRAFT fix: `$unit` is an advisory display attribute (consumers dispatch on property ID and datatype, not the unit string), so the artifact version stays 0.1 and no downstream lockfile re-pin is required; only the document Date header moved (manifest + README status table regenerated).
